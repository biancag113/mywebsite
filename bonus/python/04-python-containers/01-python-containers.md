...
1. **What are _lists_ similar to in JS?**

2. **What is a way to add items to a _list_?**

3. **What is a way to remove an item from the front of a _list_?**
...
- Unlike JS objects, Python dictionaries are *ordered* (by item insertion order)
...
- Any immutable type can be used as a key, including _numbers_ and _tuples_.
...
- Dictionaries have a `pop()` method as well, which will return the value being deleted. You are fully capable of looking that one up yourself!
